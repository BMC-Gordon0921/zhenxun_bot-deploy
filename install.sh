...
Set_pip_Mirror() {
  ...
  ${Green_font_prefix} 1.${Font_color_suffix} 清华源(默认)
  ${Green_font_prefix} 2.${Font_color_suffix} 不修改"
...
Set_check(){
echo -e "${Info} 实现原理: 通过多次运行 zhenxun_bot 并输出日志文件，检测日志文件内提示的缺少模块(例如No module named 'pypinyin')的报错内容，截取模块名称并输出进行安装。(纯属乱写一通，可能不是很稳定，介意勿用)"
check_module
}
check_module() {
check_pid_zhenxun
